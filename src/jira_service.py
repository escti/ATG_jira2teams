...
class JiraClient:
    ...
    def get_dashboard_data(self, user=None):
        ...
        queries = {
            "pessoais_aguardando": f"assignee = '{assignee_email}' AND project NOT IN (TIC, GPM) AND resolution = Unresolved AND status not in (Concluído, Backlog, Cancelado, 'PENDENTE EXTERNO') ORDER BY status ASC, 'Tempo de resolução' ASC, updated ASC",
            ...
            "pessoais_sem_interacao": f"assignee = '{assignee_email}' AND project NOT IN (TIC, GPM) AND statusCategory != Done and updatedDate <= '-3d' ORDER BY status ASC, 'Tempo de resolução' ASC, updated ASC",
            "pessoais_projetos_tic": f"assignee = '{assignee_email}' AND project IN (TIC) AND resolution = Unresolved AND status NOT IN (Concluído, Backlog, Cancelado) ORDER BY status ASC, 'Tempo de resolução' ASC, updated ASC",
            "pessoais_projetos_gpm": f"assignee = '{assignee_email}' AND project IN (GPM) AND resolution = Unresolved AND status NOT IN (Concluído, Backlog, Cancelado) ORDER BY status ASC, 'Tempo de resolução' ASC, updated ASC",
